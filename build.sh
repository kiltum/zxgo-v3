...
# run_build <name> <goos> <goarch> <cc> <cxx> <libdir> <libs> <extldflags>
#
...
#
# CXX is not optional, and it must target the same platform as CC: the vendored
# Dear ImGui makes internal/ui/imgui a C++ package, so cgo links the whole binary
# with the *C++* driver. With CXX unset that is the host `c++`, which then tries
# to link Linux (or Windows) objects and the vendored libSDL3.a with the macOS
# linker - "unknown file type in .../000000.o" and nothing else. It went unnoticed
# while the front end had no C++ in it; every cross build was broken from the day
# ImGui was vendored until this was added.
run_build() {
    local name="$1" goos="$2" goarch="$3" cc="$4" cxx="$5" libdir="$6" libs="$7"
    local extldflags="$8"
...
EOF
    ...
    GOOS="$goos" GOARCH="$goarch" CGO_ENABLED=1 CC="$cc" CXX="$cxx" \
...
build_macos() {
    ...
    # The host toolchain, both halves of it: `c++` is what clang ships as.
    run_build zxgo-macos darwin arm64 cc c++ "$libdir" "$libs" ""
}
...
build_linux() {
    ...
    local cxx="zig c++ -target x86_64-linux-gnu.$LINUX_GLIBC"
    ...
    run_build zxgo-linux linux amd64 "$cc" "$cxx" "$libdir" "-L$libdir -lSDL3 -lm -ldl -lpthread" ""
}
...
build_windows() {
    ...
    run_build zxgo-windows windows amd64 x86_64-w64-mingw32-gcc x86_64-w64-mingw32-g++ \
        "$libdir" "$libs" "-static"
}
